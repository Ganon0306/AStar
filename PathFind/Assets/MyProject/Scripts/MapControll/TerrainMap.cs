...
public class TerrainMap : TileMapController
{
    ...
    private void Start()
    {
        ...
        for(int i = 0; i < allTileObjs.Count; i++)
        {
            ...
            // 프리팹을 인스턴스화해서 교체할 차일의 트랜스폼을 복사한다.
            tempChangeTile = Instantiate(
                changeTilePrefab, transform);
    ...
    public TerrainControler GetTile(int tileIndx1D)
    {
        if(allTerrains.IsValid(tileIndx1D))
        {
            return allTerrains[tileIndx1D];
        }
        return default;
    }       // GetTile()
}
